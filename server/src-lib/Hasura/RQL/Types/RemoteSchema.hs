...
getUrlFromEnv :: (MonadIO m, MonadError QErr m) => Environment -> Text -> m N.URI
getUrlFromEnv environment urlFromEnv = do
  ...
  where
    mEnv = lookupEnv environment $ T.unpack urlFromEnv
